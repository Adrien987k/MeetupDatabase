...
def UserIndex(request):
    ...
    return render(request, 'user/user_index.html', {'now' : now})
...
def CityDetail(request, city_name):
    ...
    now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    return render(request, 'user/city_detail.html', {'city': city, 'member_count' : member_count, 'group_count' : group_count, 'venue_count' : venue_count, 'event_count' : event_count, 'now' : now})
...
def TopicDetail(request, topic_id):
    ...
    return render(request, 'user/topic_detail.html', {'topic': topic, 'parent_topic' : parent_topic, 'group_count' : group_count, 'have_parent' : have_parent, 'member_count' : member_count, 'now' : now, 'prev' : prev})
...
def GroupDetail(request, group_id):
    ...
    return render(request, 'user/group_detail.html', {'group': group, 'topics' : topics, 'organizer' : organizer,'have_org' : have_org, 'member_count' : member_count, 'now' : now, 'prev' : prev})
...
def VenueDetail(request, venue_id):
    ...
    now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    votes = range(0,6)
    return render(request, 'user/venue_detail.html', {'venue': venue, 'rating_count': rating_count, 'event_count' : event_count, 'now' : now, 'votes' : votes})

def VenueVote(request, venue_id):
    venue = get_object_or_404(Venue, pk=venue_id)
    rating_count = int(venue.rating_count)
    if rating_count == -1:
        rating_count = 0
    ...
    try:
        ...
        return render(request, 'user/venue_detail.html', {'venue': venue, 'rating_count': rating_count, 'event_count' : event_count, 'now' : now, 'error_message' : "You didn't select a choice", 'votes' : votes})
    else:
        ...
        return render(request, 'user/venue_results.html', {'venue' : venue})
...
def EventDetail(request, event_id):
    ...
    return render(request, 'user/event_detail.html', {'event': event, 'duration' : duration, 'org_group' : org_group, 'have_org' : have_org, 'venue' : venue, 'have_venue' : have_venue, 'past' : past, 'votes' : votes, 'rating_count' : rating_count})
...
def EventVote(request, event_id):
    ...
    try:
        ...
        # Redisplay the question voting form.
        return render(request, 'user/event_detail.html', {'event': event, 'duration' : duration, 'org_group' : org_group, 'have_org' : have_org, 'venue' : venue, 'have_venue' : have_venue, 'past' : past, 'rating_count' : rating_count, 'error_message' : "You didn't select a choice", 'votes' : votes})
    else:
        ...
        # with POST data. This prevents data from being posted twice if a
        # user hits the Back button.
        return render(request, 'user/event_results.html', {'event' : event})
