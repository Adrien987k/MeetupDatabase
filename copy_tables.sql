...
FROM categories2.csv csv HEADER NULL 'not_found';

\copy City(city_name,city_id,country,latitude,longitude,state)
FROM cities2.csv csv HEADER NULL 'not_found';
...
FROM topics2.csv csv HEADER NULL 'not_found';

\copy Group_topic(topic_id,group_id)
FROM groups_topics2.csv csv HEADER NULL 'not_found';

\copy Event(event_id,created,description,duration,fee_accepts,fee_amount,fee_required,group_id,headcount,how_to_find_us,maybe_rsvp_count,event_name,photo_url,rating_average,rating_count,event_status,event_time,updated,venue_id,repinned,visibility,waitlist_count,why,yes_rsvp_count)
FROM events2.csv csv HEADER NULL 'not_found';

\copy Venue(venue_id,adress_1,city_name,venue_name,rating_average,rating_count)
FROM venues.csv csv HEADER NULL 'not_found';
